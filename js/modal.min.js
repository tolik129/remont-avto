$(document).ready(function() {
    $("body").on("click", "#shadow, #alert_shadow, #confirm_shadow, .modal_close_link", function() {
...
function openModal(a) {
    if ($("div.opened_window").length !== 0) {
        $("div.opened_window").fadeOut(300)
    } else {
        $("#shadow").height($(document).height()).fadeIn(300)
    }
    $("#" + a).addClass("opened_window");
    modalPosition();
    $("#" + a).fadeIn(300)
}
...
function closeModal() {
    $("div.opened_window").fadeOut(300);
    $("#shadow").fadeOut(300);
    $("div.opened_window").removeClass("opened_window");
    $(".login_err").text("")
}

function myAlert(d, c, b) {
    ...
    var a = '<div id="alert_modal" class="modal_window small_modal_window alert_type_' + b + '"><div class="modal_wrapper"><div class="modal_close"><a href="javascript:void(0)" title="' + _("Close") + '" class="modal_close_link" onclick="closeMyAlert()">&times;</a></div><div class="modal_top"><div class="modal_title">' + c + '</div></div><div class="modal_content"><p class="text" id="modal_text">' + d + '</p><div class="modal_button"><a href="javascript:void(0)" class="btn btn-green" onclick="closeMyAlert()">' + _("Рћk") + '</a></div></div></div></div><div id="alert_shadow"></div>';
    if ($("#alert_modal").length != 0) {
        $("#alert_modal").remove()
    }
    $("body").append(a);
    $("#alert_shadow").height($(document).height()).css({
        opacity: "0",
        display: "block"
    });
    $("#alert_shadow").animate({
        opacity: "0.65"
    }, 500);
    $("#alert_modal").addClass("opened_window");
    modalPosition();
    $("#alert_modal").fadeIn(300)
}

function closeMyAlert() {
    $("#alert_modal").fadeOut(300).removeClass("opened_window");
    $("#alert_shadow").animate({
        opacity: "0"
    }, 500);
...
function myConfirm(e, c, a, d) {
    ...
    if (typeof(c) == "undefined") {
        c = myAlert(_("Nice guy! :)"))
    }
    var b = '<div id="confirm_modal" class="modal_window small_modal_window"><div class="modal_wrapper"><div class="modal_close"><a href="javascript:void(0)" title="' + _("Close") + '" class="modal_close_link" onclick="closeMyConfirm()">&times;</a></div><div class="modal_top"><div class="modal_title">' + d + '</div></div><div class="modal_content"><div class="text" id="modal_text">' + e + '</div><div class="modal_button"><a href="javascript:void(0)" class="btn btn-blue btn-smaller" onclick="closeMyConfirm()">' + _("Cancel") + '</a><a href="javascript:void(0)" class="btn btn-orange btn-smaller" onclick="' + c + '">' + a + '</a></div></div></div></div><div id="confirm_shadow"></div>';
    if ($("#confirm_modal").length != 0) {
        $("#confirm_modal").remove()
    }
    $("body").append(b);
    $("#confirm_shadow").height($(document).height()).css({
        opacity: "0",
        display: "block"
    });
    $("#confirm_shadow").animate({
        opacity: "0.65"
    }, 500);
    $("#confirm_modal").addClass("opened_window");
    modalPosition();
    $("#confirm_modal").fadeIn(300)
}

function closeMyConfirm() {
    $("#confirm_modal").fadeOut(300).removeClass("opened_window");
    $("#confirm_shadow").animate({
        opacity: "0"
    }, 500);
...
function modalPosition() {
    var a = $("div.opened_window");
    var c = ($(window).height() - a.height()) / 2;
    var b = ($(window).width() - a.width()) / 2;
    a.css({
        top: c,
        left: b
    })
...
$(document).ready(function(){$(".album_1").colorbox({rel:"album_1",maxHeight:"95%"})});
...
function topPanel(e, c, d, b) {
    ...
    var a = '<div id="toppanel" style="display:none" class="toppanel_' + c + '"><div class="toppanel_wrapper">';
    if (d == false) {
        a += '<a href="javascript:void(0)" id="toppanel_close" onclick="closeTopPanel()">&times;</a>'
    }
    a += "<p>" + e + "</p></div></div>";
    ...
    $("#toppanel").fadeIn(300);
    if (d) {
        setTimeout("closeTopPanel()", b)
    }
}

function closeTopPanel() {
    $("#toppanel").fadeOut(300);
    setTimeout(function() {
        $("#toppanel").remove()
    }, 300)
};
...
$(document).ready(function() {
    ...
    if ($("input[type='phone']").length != 0) {
        $("input[type='phone']").mask("+38 (099) 999-99-99")
    }
    if ($(".tooltip").length != 0) {
        $(".tooltip").tooltipster()
    }
    if ($(".modal_cart_wrap").length != 0) {
        $(".modal_cart_wrap").mCustomScrollbar({
            axis: "y",
            theme: "dark"
        })
    }
    ...
    $("input.required").blur(function() {
        if ($(this).val() != "") {
            $(this).parents("div").removeClass("has_err")
        }
    });
    $("#menu_hamburger>.hamburger").click(function() {
        if ($(this).hasClass("is-active")) {
            $(this).removeClass("is-active");
            $("#menu_shadow").fadeOut(200);
            $("#mobile_menu").animate({
                left: "-380px"
            }, 200)
        } else {
            $(this).addClass("is-active");
            $("#menu_shadow").fadeIn(200);
            $("#mobile_menu").animate({
                left: 0
            }, 200)
        }
    ...
    $("#menu_shadow").click(function() {
        $("#menu_hamburger>.hamburger").removeClass("is-active");
        $(this).fadeOut(200);
        $("#mobile_menu").animate({
            left: "-380px"
        }, 200)
    });
    $("#mobile_menu li.has_sub a.menu_toggle").click(function(a) {
        $(this).next("ul").slideToggle(200)
    });
    $("#catalog_hamburger>.hamburger").click(function() {
        if ($(this).hasClass("is-active")) {
            $(this).removeClass("is-active");
            $(".catalog_nav").slideUp(200)
        } else {
            $(this).addClass("is-active");
            $(".catalog_nav").slideDown(200)
        }
    });
    ...
    if (document.cookie.indexOf("right_hidden=toggled") != -1) {
        $("#fixed_right").addClass("toggled")
    }
    $(".toggle_fixed_right").click(function() {
        if ($("#fixed_right").hasClass("toggled")) {
            $("#fixed_right").removeClass("toggled");
            setCookie("right_hidden", "showed")
        } else {
            $("#fixed_right").addClass("toggled");
            setCookie("right_hidden", "toggled")
        }
    });
    new WOW().init();
    if ($("main table.table").length != 0) {
        $("main table.table").wrap('<div class="table_wrapper"></div>')
    }
});
